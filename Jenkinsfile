...
//             sh "kubectl apply -f ${EKS_DEPLOYMENT_FILE} -n ${EKS_NAMESPACE}"
            sh "kubectl set image deployment.apps/${EKS_DEPLOYMENT_NAME} ${RUNNING_CONTAINER_NAME}=${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${AWS_ECR_IMAGE}:${IMAGE_VERSION} -n ${EKS_NAMESPACE}"
